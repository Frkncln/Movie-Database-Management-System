...
function display_query($Query, $SubQuery,$Result_Type) {
	$con=mysqli_connect("localhost","root","cel78en","moviedbv4");
	
	//display the query of page
	...
	if($Result_Type == "deleteactor"){
			echo "\n Couldnt find specified actor.";}
	elseif($Result_Type == "adddire"){
		echo "\n Person with the same details exists. Same person is added to Directors list";}
	elseif($Result_Type == "addmovie"){
			echo "\n Movie with the same details exists. Same movie is added to Movies list";}
	elseif($Result_Type == "updatemovie"){
				echo "\n Movie with the same details exists. Same movie is added to Movies list";}
